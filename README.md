...
A Streamlit dashboard that renders the prophetic knowledge graph as a live InfraNodus iframe while serving statistics from a cached JSON export for instant performance. No more 30-second API timeouts.
...
No more blank graphs. The network visual is served directly by InfraNodus, not rebuilt from raw JSON.
No more timeouts. All metrics, top nodes, edges, clusters, gaps, and statements come from a local file that ships with the repo.
Optional live refresh. Toggle Refresh live stats from API in the sidebar to overlay fresh summary data from POST /graphAndStatements (with includeGraph=false to stay fast).
...
ACCESS_PASSWORD = "R******"
INFRANODUS_API_KEY = "****"
INFRANODUS_API_KEY is optional — diagnostics and the live-stats toggle require it, but the dashboard itself runs from the cached JSON alone.
...
Copy"Layer 3 (Kairos Transition)": {
    "graph_name": "layer_3",
